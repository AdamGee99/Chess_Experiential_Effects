...
// pretty much identical to fit 4 but new interaction term - trying to capture the effect of the pairs of opponents
// not putting any correlation between the paired effects - would get too messy I think
// is this over parameterized though - in this case we don't even need a player and opponent effect...
...
parameters {
  ...
  matrix[J, J] delta;                  // paired effects
...
model {
  ...
  vector[N] pred;
  for(i in 1:N){
    pred[i] = beta[1, id_focal[i]] - beta[1, id_opp[i]] + delta[id_focal[i], id_opp[i]] + beta[2, id_focal[i]] * win_prop[i] +
