// March 27, 2025
// player and opponent effects
// pretty much identical to fit 2 but adding in an opponent effect 
...
model {
  ...
  vector[N] pred;
  for(i in 1:N){
    pred[i] = beta[1, id_focal[i]] - beta[1, id_opp[i]] + beta[2, id_focal[i]] * win_prop[i] +
