...
model_path = here("results", "model fits", "opponent_effect", "fit_opp_effect_gm.RDS")
...
bullet_60 = lichess_data %>% 
  ...
  filter(White %in% users & Black %in% users) #filter for closed system when using two player effects

#remove the duplicate rows
bullet_60_no_dup = bullet_60[!duplicated(bullet_60),]

#final 1000 games each user plays
#doing 250 games since so little data
...
focal_player_used = which(users == "DrNykterstein")
...
focal_draws = model$draws() |> as_draws_df() |>
  dplyr::select(starts_with(c("alpha[", "beta[", "gamma"))) |>
  mutate(draw = row_number()) |> 
  pivot_longer(cols = "alpha[1]":"gamma") |>
...
focal_draws$gamma = model$summary("gamma") %>% pull(mean)
...
for (i in 1:num_games) {
  ...
  gamma = game_info$color
  
  # get opponent name and ability
  opp = focal_init_data[i,] |> 
    mutate(opp_name = ifelse(focal_white == 1, Black, White)) |> 
    pull(opp_name)
  opp_id = match(opp, users)
  opp_alpha = model$summary(paste0("alpha[",opp_id ,"]")) %>% pull(mean) #just use the mean alpha for opponent
  ...
  for (j in 1:num_sims) {
    ...
    inv_logit <- curr_draw$alpha - opp_alpha + curr_draw$beta * curr_prop +
      curr_draw$gamma * gamma
...
sim_results = foreach(sim = 1:num_sims, .combine = "cbind", .packages = c("tidyverse", "doParallel", "rstanarm")) %dopar% {
  ...
  foreach(game = 1:num_games, .combine = "c") %do% {
    ...
    #find probability
    lin_comb = sim_draw$alpha - opp_alpha +
      (sim_draw$beta * curr_hist) +
      (sim_draw$gamma * colour_ind)
...
#the true number of wins on the final games
...
ggsave(plot = wins_ppd,
       filename = here("results", "temp model figures", fit, "ppd.png"), dpi = 1000,
       height = 6, width = 7)
