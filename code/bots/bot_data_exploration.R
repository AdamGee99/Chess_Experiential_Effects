...
engine_blitz_raw = read.pgn(here("data", "bot data", "TCEC_Season_27_-_Division_P.pgn"))
...
#table of current result and previous result for berserk

#berserk - huge winner effect
...
#slowchess - no winner effect
init_data %>% filter(focal_user == users[8]) %>% 
  mutate(prev_result  = as.numeric((round(ave_prop*2) + 1) /2)) %>%
  mutate(result = focal_result) %>%
  select(result, prev_result) %>%
  table()
...
#torch - winner effect
init_data %>% filter(focal_user == users[10]) %>% 
  mutate(prev_result  = as.numeric((round(ave_prop*2) + 1) /2)) %>%
  mutate(result = focal_result) %>%
  select(result, prev_result) %>%
  table()

#stockfish - winner effect
...
# num_games = init_data %>% filter(focal_user == users[8]) %>% mutate(prev_result  = ((round(ave_prop*2) + 1) /2)) %>% filter(prev_result != 0.5) %>% nrow()
# focal_mean_result = init_data %>% filter(focal_user == users[8]) %>% mutate(prev_result  = ((round(ave_prop*2) + 1) /2)) %>% filter(prev_result != 0.5) %>% pull(focal_result) %>% mean()
# init_data %>% filter(focal_user == users[8]) %>% 
#   mutate(prev_result  = ((round(ave_prop*2) + 1) /2)) %>%
#   mutate(result = focal_result) %>%
#   select(result, prev_result) %>%
#   filter(prev_result != 0.5) %>% #the result of the current game when previous result is a draw shouldn't impact winner effect
#   table()/num_games
...
stan_file <- here("code", "stan", "bot_fit4.stan")
fit = "fit5"
...
#refer to readme in model fits folder for info on each fit
